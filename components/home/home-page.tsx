...
const tonsText = ({ t }: Translator, value: number) =>
  t('{tons} Tons', { tons: value.toLocaleString('en-US', { maximumFractionDigits: 1 }) });
  ...
  if (current === 0 && previous === 0) {
    return (
      <p className="hm-delta" data-trend="flat">
        ...
        {period === 'week' ? t('None last week either') : t('None last month either')}
      </p>
    );
  }
...
export default function HomePage() {
  ...
  const topLoads = Math.max(1, ...topCustomers.map((entry) => entry.totals.loads));
  const fleet = trucks
    .map((truck) => ({
      truck,
      summary: summarize(
  ...
  if (drafts.length) {
    attention.push({
      key: 'drafts',
      tone: 'warning',
      title: t('{invoices} waiting for a rate', { invoices: plural(drafts.length, 'invoice') }),
      detail: `${drafts
        .slice(0, 3)
  ...
  if (review.length) {
    attention.push({
      key: 'review',
      tone: 'warning',
      title: t('{tickets} to double-check', { tickets: plural(review.length, 'ticket') }),
  ...
  const steps = [
    {
      ...
      detail: t('Pick a truck when uploading and its number goes on the invoice.'),
      href: '/fleet',
      action: t('Add trucks'),
    },
    {
  ...
  return (
    <>
      ...
      {!ready ? (
        ...
        <>
          {setupDone ? null : (
            <section className="ld-panel hm-setup" aria-labelledby="hm-setup-title">
              ...
              <ol className="hm-steps">
                {steps.map((step) => (
                  <li key={step.key} className="hm-step" data-done={step.done}>
                    <p className="hm-step-title">
                      ...
                      {step.title}
                      <span className="sr-only">{step.done ? ` (${t('done')})` : ''}</span>
                    </p>
                    <p>{step.detail}</p>
                    {step.done ? null : (
                      <Link
          ...
          <div className="hm-stats">
            <StatTile
              ...
              sub={t('{tons} hauled', { tons: tonsText(tr, compare.week.tons) })}
            >
              <Delta
            ...
            <StatTile
              ...
              sub={t('{tons} hauled', { tons: tonsText(tr, compare.month.tons) })}
            >
              <Delta
            ...
            >
              {drafts.length + review.length === 0 ? (
                ...
                <p className="hm-delta" data-trend="down">
                  ...
                  {plural(drafts.length, 'draft')} · {plural(review.length, 'ticket')}
                </p>
              )}
            </StatTile>
          </div>
          ...
          <div className="hm-grid">
            ...
            <section className="ld-panel" aria-labelledby="hm-attention-title">
              <div className="ld-panel-head">
                <div>
              ...
              {attention.length ? (
                <ul className="hm-attention">
                  {attention.map((item) => (
                    <li key={item.key}>
                      <span className="hm-attention-icon" data-tone={item.tone}>
                        {item.tone === 'warning' ? (
                          <CircleAlert aria-hidden="true" />
                        ) : item.key === 'trucks' ? (
          ...
          <div className="hm-grid-3">
                  ...
                  {groups.slice(0, 5).map((group) => (
                    <li key={group.key}>
                      <button
                        ...
                        onClick={() =>
                          setInvoiceView({
                            lines: invoiceLines(records, group.invoice.invoice_number),
            ...
            <section className="ld-panel" aria-labelledby="hm-customers-title">
              <div className="ld-panel-head">
                <div>
                  <p className="ld-step">
                  ...
                  {topCustomers.map((entry) => (
                    <li key={entry.key}>
                      <div className="hm-meter-head">
                        ...
                        <span>
                          {plural(entry.totals.loads, 'load')} · {money(entry.totals.billed)}
                        </span>
                      </div>
                      <div className="hm-meter" aria-hidden="true">
                        <span style={{ width: `${(entry.totals.loads / topLoads) * 100}%` }} />
                      </div>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="ld-empty">{t('Customers appear once tickets are saved.')}</p>
              )}
            </section>
              ...
              {fleet.length ? (
                <ul className="hm-list">
                  {fleet.map(({ truck, summary }) => (
                    <li key={truck.id} className="hm-row" data-inactive={!truck.active}>
                      <span className="hm-row-main">
                ...
                <p className="ld-empty">
                  {t('No trucks yet.')} <Link href="/fleet">{t('Add your trucks')}</Link>{' '}
