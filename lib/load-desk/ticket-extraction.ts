...
const FIELD_NOTES: Record<ObservedFieldName, string> = {
  ...
  time_in: 'the time in (arrival at the plant), as printed, where one is printed',
  time_out: 'the time out (departure from the plant), as printed, where one is printed',
  location:
 ...
 * for anything that is not a time of day. Mileage orders a day's loads by
 * these, so a word or a broken figure is left null rather than guessed at.
 */
